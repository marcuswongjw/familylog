// ============================================================
// Wong Family — Full App Logic (External)
// ============================================================
...
function loadPreference(key, fallback) {
  try {
    const val = localStorage.getItem('wf_' + key);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Restore preferences
  calView = loadPreference('calView', 'list');
  activeExpenseAccount = loadPreference('expenseAccount', 'All');
  activeBudgetAccount = loadPreference('budgetAccount', 'All');
  schedView = loadPreference('schedView', 'list');
  activeSchedChild = loadPreference('schedChild', 'Mikaela');
  const savedDate = loadPreference('selectedCalDay', '');
  if (savedDate) selectedCalDayStr = savedDate;

  // Apply restored view states
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/sw.js')
      ...
      .catch(err => console.warn('❌ SW registration failed (ignore):', err));
  }
...
function showUndoToast(message) {
  const item = _undoStack[_undoStack.length - 1];
  if (!item) return;
  const el = document.getElementById('toast');
  el.innerHTML = `${message || item.message} <button onclick="undoLast()" style="background:none;border:1px solid #fff;color:#fff;border-radius:4px;padding:2px 8px;margin-left:8px;cursor:pointer;">Undo</button>`;
  el.classList.add('show');
  clearTimeout(toastT);
  toastT = setTimeout(() => {
    el.classList.remove('show');
    // If not undone, remove from stack after the toast disappears
    setTimeout(() => { if (_undoStack.length) _undoStack.pop(); }, 500);
  }, 5000);
}

function pushUndo(restoreFn, message) {
  _undoStack.push({ restore: restoreFn, message: message || 'Item deleted' });
...
function buildLogin() {
  ...
  grid.innerHTML = MEMBERS.map(m =>
    `<button class="member-btn" onclick="selectMember('${m.name}')">
      <span class="av">${m.emoji}</span>${m.name}
    </button>`
  ).join('');
}
...
function selectMember(name) {
  ...
  document.querySelectorAll('.member-btn').forEach(b => {
    if (b.textContent.trim() === name) b.classList.add('sel');
  });
  const pinbox = document.getElementById('pinbox');
  pinbox.style.display = 'block';
  document.getElementById('pinlbl').textContent = name + "'s password";
  document.getElementById('login-error').textContent = '';
  document.getElementById('login-email').value = selectedMember.email;
  setTimeout(() => document.getElementById('login-password').focus(), 100);
}

function loginWithSelectedMember() {
  if (!selectedMember) {
    document.getElementById('login-error').textContent = 'Please select a member first.';
    return;
  }
...
function handleLogin(event) {
  ...
  const password = document.getElementById('login-password-manual').value;
  const errorEl = document.getElementById('login-error');
...
function logout() {
  firebase.auth().signOut().then(() => {
    ...
    document.getElementById('login-password-manual').value = '';
    document.getElementById('login-error').textContent = '';
    selectedMember = null;
    document.querySelectorAll('.member-btn').forEach(b => b.classList.remove('sel'));
  ...
  return new Promise((resolve) => {
    ...
    currentUser.getIdToken().then(idToken => {
      ...
      window[cb] = r => {
        ...
        data = r || {};
        if (r && r.expenseGroups) GROUPS = r.expenseGroups;
      ...
      s.onerror = () => {
        delete window[cb];
        s.remove();
        showError('Network error – could not load data');
        finishProgressBar();
        resolve();
      };
      s.src = GAS_URL + '?action=get_all&idToken=' + encodeURIComponent(idToken) + '&callback=' + cb;
...
function gPost(payload) {
  ...
  return new Promise((resolve) => {
    ...
    currentUser.getIdToken().then(idToken => {
      ...
      s.src = GAS_URL + '?action=write&idToken=' + encodeURIComponent(idToken) + '&callback=' + cb + '&data=' + encodeURIComponent(JSON.stringify(payload));
      document.body.appendChild(s);
    }).catch(err => {
...
function buildDynamicSelectors() {
  ...
  if (rgs) {
    rgs.innerHTML = Object.keys(GROUPS).map(g => `<option value="${g}">${g}</option>`).join('');
    updateRcCats();
  }

  const egs = document.getElementById('ex-grp');
  if (egs) {
    egs.innerHTML = Object.keys(GROUPS).map(g => `<option value="${g}">${g}</option>`).join('');
    updateExCats();
  }
}

function updateRcCats() {
  const grp = document.getElementById('rc-grp');
  const cat = document.getElementById('rc-cat');
  if (!grp || !cat) return;
  const g = grp.value;
  cat.innerHTML = (GROUPS[g] || []).map(c => `<option value="${c}">${c}</option>`).join('');
}

function updateExCats() {
  const grp = document.getElementById('ex-grp');
...
function goTo(id) {
  ...
  if (FAB_MAP[id]) { fab.classList.remove('hide');
    fab._m = FAB_MAP[id]; } else fab.classList.add('hide');
  render(id);
}
...
function chips(id, opts, def, key) {
  ...
  el.innerHTML = opts.map(o =>
    `<span class="chip${o === def ? ' sel' : ''}" data-k="${key}" data-v="${o}" onclick="selChip(this,'${key}')">${o}</span>`
  ).join('');
}
...
function openM(id) {
  ...
  if (id === 'm-event') {
    ...
    if (el) el.value = selectedCalDayStr || todayStr();
    chips('ev-chips', FAM, 'Everyone', 'ev-tag');
  }
}
...
async function submitBirthday(btn) {
  ...
  btn.textContent = 'Saving…';
  try {
    const name = v('bd-name');
    const month = v('bd-month');
    const day = v('bd-day');
    ...
    closeM('m-birthday');
    clr('bd-name', 'bd-day', 'bd-year', 'bd-notes');
    toast('Saved! 🎂');
    await loadData();
  } finally {
    btn.disabled = false;
...
async function submitBudget(btn) {
  ...
  btn.textContent = 'Saving…';
  try {
    const group = v('bud-grp');
    const amt = parseFloat(v('bud-amt'));
...
async function submitRecurring(btn) {
  ...
  btn.textContent = 'Saving…';
  try {
    const name = v('rc-name');
    const amt = parseFloat(v('rc-amt'));
    const day = parseInt(v('rc-day'));
...
async function submitExpense(btn) {
  ...
  btn.textContent = 'Saving…';
  try {
    const desc = v('ex-desc');
    const amt = parseFloat(v('ex-amt'));
    const date = v('ex-date');
...
async function submitSchedule(btn) {
  ...
  try {
    ...
    clr('sch-act', 'sch-time', 'sch-end-time', 'sch-loc', 'sch-notes');
    toast('Schedules updated! ⛵');
    await loadData();
    renderSchedules();
  } finally {
    btn.disabled = false;
    btn.textContent = 'Save to Calendar';
  }
}
...
async function submitAppreciation(btn) {
  ...
  try {
    ...
    if (res && res.status === 'ok') { toast('Note dropped in the jar! 🍯');
      await loadData(); } else toast('Error saving note');
  } finally {
    btn.disabled = false;
    btn.textContent = 'Drop in Jar 🍯';
  }
}
...
async function submitLoveCheckin(btn) {
  ...
  try {
    ...
    if (res && res.status === 'ok') { toast('Check-in saved! 😊');
      await loadData(); } else toast('Error saving check-in');
  } finally {
    btn.disabled = false;
...
function renderHome() {
  ...
  document.getElementById('greet').innerHTML =
    `<h2>${g}, ${escapeHtml(user)}! 👋</h2><p>${new Date().toLocaleDateString('en-SG', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p>`;
  const tod = todayStr();
  ...
  document.getElementById('today-wrap').innerHTML = `
    <div class="card" style="margin-top:14px;">
      ...
      <div class="card-body">
        ...
        ${evs.map(e => `<div class="row"><div style="font-size:16px">📅</div><div class="row-main"><div class="row-title">${escapeHtml(e.title)}</div><div class="row-sub">${escapeHtml(e.time)}</div></div></div>`).join('')}
        ${tks.map(t => `<div class="row"><div style="font-size:16px">⚠️</div><div class="row-main"><div class="row-title">${escapeHtml(t.task)}</div><div class="row-sub">Due today · ${escapeHtml(t.assignee)}</div></div></div>`).join('')}
      </div>
    </div>`;
...
let selectedCalDayStr = loadPreference('selectedCalDay', new Date().toISOString().slice(0, 10));
...
function prevCalMonth() { calMonth--; if (calMonth < 0) { calMonth = 11;
    calYear--; } renderCal(); }

function nextCalMonth() { calMonth++; if (calMonth > 11) { calMonth = 0;
    calYear++; } renderCal(); }

function prevCalWeek() { calWeekStart.setDate(calWeekStart.getDate() - 7);
  renderCal(); }

function nextCalWeek() { calWeekStart.setDate(calWeekStart.getDate() + 7);
  renderCal(); }

function snapCalWeekToday() { calWeekStart = getStartOfWeek(new Date());
  renderCal(); }

function snapCalMonthToday() { const d = new Date();
  calMonth = d.getMonth();
  calYear = d.getFullYear();
  selectedCalDayStr = d.toISOString().slice(0, 10);
  savePreference('selectedCalDay', selectedCalDayStr);
  renderCal(); }
    ...
    if (dayEvents.length > 0) {
      eventsHtml = dayEvents.map(e => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px dashed #f1f2f6;">
          <div style="flex:1;">
            <div style="font-weight:600;font-size:13px;color:#1a1a2e;">${escapeHtml(e.title)}</div>
            <div style="font-size:11px;color:#9898aa;">${escapeHtml(e.time)}${e.endTime ? ' - ' + escapeHtml(e.endTime) : ''}${e.location ? ' · ' + escapeHtml(e.location) : ''}</div>
            ${e.notes ? `<div style="font-size:11px;color:#888;margin-top:2px;font-style:italic;">"${escapeHtml(e.notes)}"</div>` : ''}
...
function renderCalMonth() {
  ...
  for (let day = 1; day <= numDays; day++) {
    ...
    if (dayEvents.length > 0) {
      ...
      dayEvents.forEach(e => {
        if (e.tags && e.tags.length > 0) e.tags.forEach(t => uniqueTags.add(t));
        else uniqueTags.add('Everyone');
      });
...
function delEvent(id) {
  ...
  renderCal();
  renderHome();
  // Actually delete from backend in background
...
function delTask(n) {
  ...
  renderTasks();
  renderHome();
  gPost({ note: 'delete_todo', todo_id: n });
  toast('Task deleted (undo available)');
}
...
function delExp(n) {
  ...
  pushUndo(() => {
    data.expenses.rows.push(exp);
    data.expenses.total += exp.amount;
    renderExpenses();
  }, 'Expense deleted');
  renderExpenses();
  gPost({ note: 'delete_expense', row_id: n });
  toast('Expense deleted (undo available)');
}

// ─── DELETE: Recurring (with Undo) ──────────────────────────────
function delRec(n) {
  ...
  renderRecurring();
  gPost({ note: 'delete_recurring', row_num: n });
  toast('Recurring removed (undo available)');
}

// ─── DELETE: Schedule (with Undo) ──────────────────────────────
...
function delTrip(id) {
  ...
  pushUndo(() => {
    data.travel.push(trip);
    renderTravel();
  }, 'Trip removed');
  renderTravel();
  gPost({ note: 'delete_trip', trip_id: id });
  toast('Trip removed (undo available)');
}
...
function renderTasks() {
  ...
  const g = {};
  FAM.forEach(m => g[m] = []);
  todos.forEach(t => { if (!g[t.assignee]) g[t.assignee] = [];
    g[t.assignee].push(t); });
  el.innerHTML = FAM.filter(m => g[m] && g[m].length).map(m =>
    `<div style="padding:8px 16px 2px;font-size:11px;font-weight:700;color:#9898aa;text-transform:uppercase;letter-spacing:.5px;">${escapeHtml(m)}</div>` +
    g[m].map(t => {
      const today = todayStr();
      let dueHtml = 'No due date';
      if (t.due) {
        if (t.dueRaw < today) dueHtml = `<span style="color:#b83232;font-weight:600;">⚠️ Overdue · ${escapeHtml(t.due)}</span>`;
        else if (t.dueRaw === today) dueHtml = `<span style="color:#a3600a;font-weight:600;">⏳ Due today</span>`;
        else dueHtml = `Due ${escapeHtml(t.due)}`;
      }
      return `
      <div class="task-row">
...
function toggleExpenseAccount(acc) {
  ...
  savePreference('expenseAccount', acc);
  renderExpenses();
}
...
function renderExpenses() {
  ...
  cats.forEach((c, i) => {
    ...
    donutSvgCircles +=
      `<circle cx="21" cy="21" r="15.91549430918954" fill="transparent" stroke="${color}" stroke-width="5.5" stroke-dasharray="${strokeDashArray}" stroke-dashoffset="${strokeDashOffset}"></circle>`;
    currentPercent += pct;
  });
  if (cats.length === 0) donutSvgCircles =
    `<circle cx="21" cy="21" r="15.91549430918954" fill="transparent" stroke="#e4e6ef" stroke-width="5.5"></circle>`;
  let momHtml = '';
  if (lastMonth > 0) {
    const diff = total - lastMonth;
    const pctDiff = (diff / lastMonth) * 100;
    if (diff > 0) momHtml =
      `<span class="badge b-red" style="margin-left:8px;">📈 +${pctDiff.toFixed(1)}% vs last month ($${lastMonth.toFixed(0)})</span>`;
    else momHtml =
      `<span class="badge b-green" style="margin-left:8px;">📉 ${Math.abs(pctDiff).toFixed(1)}% vs last month ($${lastMonth.toFixed(0)})</span>`;
  } else momHtml = `<span class="badge b-blue" style="margin-left:8px;">New Tracker</span>`;
  ...
  const barChartHtml = hist.map((h, i) => {
    ...
    return `
      <div class="bar-col">
        ...
        <div class="bar-lbl">${escapeHtml(h.label)}</div>
      </div>`;
  }).join('');
  el.innerHTML = `
        ...
        <div class="pie-wrap" style="flex:1;padding:0;gap:6px;">
          ${cats.slice(0, 5).map((e,i) => `
            <div class="pie-row">
              ...
              <span class="pie-lbl" style="font-size:12px;">${escapeHtml(e[0].split(' - ').pop())}</span>
    ...
    <div class="card">
      ...
      <div class="card-body">${(filteredRows||[]).slice(0, 15).map(r => `
        <div class="row">
          <div class="row-main">
            <div class="row-title">${escapeHtml(r.desc||r.category)}</div>
            <div class="row-sub">${escapeHtml(r.date)} · ${escapeHtml(r.category)} · ${escapeHtml(r.account)}</div>
          </div>
    ...
    budgetsHtml = filteredBuds.sort((a, b) => a.group.localeCompare(b.group)).map(b => {
      ...
      return `<div class="bud-row" style="border-bottom:1px solid #e4e6ef;padding:12px 0;">
        <div class="bud-hdr" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
          <span class="bud-name" style="font-weight:600;">${escapeHtml(b.group)}</span>
          <div style="display:flex;align-items:center;gap:8px;">
        ...
        <div style="font-size:11px;color:#9898aa;margin-bottom:6px;">Account: ${escapeHtml(b.account || 'Family')}</div>
...
function renderMemories() {
  ...
  el.innerHTML = mems.map(m => `
    <div class="mem-row">
      <div class="mem-type">${te(m.type)} ${escapeHtml(m.type.split(' ').slice(1).join(' '))} · ${escapeHtml(m.person)}</div>
      <div class="mem-text">${escapeHtml(m.memory)}</div>
      <div class="mem-meta">📅 ${escapeHtml(m.date)} · logged by ${escapeHtml(m.loggedBy)}</div>
    </div>`).join('');
}
...
function renderFertility() {
  ...
  el.innerHTML = rows.map(r => `<div class="fert-stat"><span class="fert-lbl">${r[0]}</span><span class="fert-val">${escapeHtml(r[1]||'—')}</span></div>`).join('') +
    ((f.symptoms || []).length ?
      `<div style="padding:12px 16px;"><div style="font-size:11px;font-weight:700;color:#9898aa;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;">Recent Symptoms</div>${f.symptoms.map(s=>`<div style="font-size:14px;padding:4px 0;border-bottom:1px solid #e4e6ef;">${escapeHtml(s.date)} · ${escapeHtml(s.note)}</div>`).join('')}</div>` :
      '');
}
...
function renderRecurring() {
  ...
  el.innerHTML = items.map(r => {
    const w = r.daysLeft === 0 ? '<span class="badge b-red">Today</span>' : r.daysLeft === 1 ?
    ...
    return `<div class="rec-row">
      <div class="rec-main">
        ...
        <div class="rec-meta">${escapeHtml(r.category)} · ${r.day}${sfx(r.day)} of month</div>
      </div>
      <div class="rec-right">
...
function renderBirthdays() {
  ...
  el.innerHTML = items.map(b => {
    ...
    const w = b.daysAway === 0 ? '<span class="badge b-red">Today! 🥳</span>' : b.daysAway === 1 ?
      '<span class="badge b-amber">Tomorrow!</span>' : b.daysAway <= 7 ?
      `<span class="badge b-amber">In ${b.daysAway} days</span>` :
      `<span class="badge b-gray">In ${b.daysAway} days</span>`;
    return `<div class="bd-row">
      <div class="bd-ic">${ic}</div>
      <div class="bd-main">
        <div class="bd-name">${escapeHtml(b.name)}</div>
        <div class="bd-meta">${escapeHtml(b.dateLabel)}${b.agePart?' · '+escapeHtml(b.agePart):''}${b.notes?' · '+escapeHtml(b.notes):''}</div>
      </div>
      <div>${w}</div>
    </div>`;
  }).join('');
...
function filterSchedules(child) {
  ...
  savePreference('schedChild', child);
...
function prevSchedMonth() { schedMonth--; if (schedMonth < 0) { schedMonth = 11;
    schedYear--; } renderSchedules(); }

function nextSchedMonth() { schedMonth++; if (schedMonth > 11) { schedMonth = 0;
    schedYear++; } renderSchedules(); }

function selectGridDay(dayStr) { selectedGridDayStr = dayStr;
  renderSchedules(); }
...
function renderSchedules() {
    ...
    timetableHtml = upcoming.slice(0, 10).map(c => {
      ...
      return `
        <div class="row" style="padding:10px 16px;">
          <div style="font-size:20px;flex-shrink:0;margin-right:6px;">⛵</div>
          <div class="row-main">
            <div class="row-title" style="font-weight:600;">${escapeHtml(c.title)}</div>
            <div class="row-sub">${escapeHtml(dayName)}, ${escapeHtml(c.date)} · ${escapeHtml(c.time)}${c.endTime ? ' - ' + escapeHtml(c.endTime) : ''}${c.location ? ' · ' + escapeHtml(c.location) : ''}</div>
            ${c.notes ? `<div style="font-size:11px;color:#888;margin-top:2px;font-style:italic;">"${escapeHtml(c.notes)}"</div>` : ''}
          </div>
  ...
  let breakdownItems = '';
  for (const act in activityBreakdown) breakdownItems +=
    `<div style="font-size:12px;color:#5a5a72;margin-top:4px;"><strong>${escapeHtml(act)}:</strong> ${activityBreakdown[act].toFixed(1)} hrs</div>`;
  const summaryHtml = `
  ...
  let timelineHtml = past.slice(0, 10).map(a => `
    <div class="row" style="padding:10px 16px;">
      <div style="font-size:20px;flex-shrink:0;margin-right:6px;">✅</div>
      <div class="row-main">
        <div class="row-title" style="font-weight:600;">${escapeHtml(a.title)} (${a.duration.toFixed(1)} hrs)</div>
        <div class="row-sub">${escapeHtml(a.date)} · Completed ${a.location ? ' · ' + escapeHtml(a.location) : ''}${a.notes ? ' · ' + escapeHtml(a.notes) : ''}</div>
      </div>
...
function renderSchedulesGrid() {
  ...
  let html = `
    ...
    <div class="cal-grid">
  `;
  for (let i = startDayOfWeek - 1; i >= 0; i--) { const dVal = prevMonthDays - i;
    html += `<div class="cal-day-cell other-month"><span class="cal-day-num">${dVal}</span></div>`; }
  const today = todayStr();
  for (let day = 1; day <= numDays; day++) {
  ...
  if (selectedDayEvents.length > 0) {
    selectedDayHtml = selectedDayEvents.map(c => `
      <div class="row" style="padding:10px 16px;">
        ...
        <div class="row-main">
          <div class="row-title" style="font-weight:600;">${escapeHtml(c.title)}</div>
          <div class="row-sub">${escapeHtml(c.time)}${c.endTime ? ' - ' + escapeHtml(c.endTime) : ''}${c.location ? ' · ' + escapeHtml(c.location) : ''}</div>
          ${c.notes ? `<div style="font-size:11px;color:#888;margin-top:2px;font-style:italic;">"${escapeHtml(c.notes)}"</div>` : ''}
        </div>
        <button onclick="delSchedule('${c.id}')" style="color:#9898aa;font-size:16px;padding:4px;flex-shrink:0;border:none;background:transparent;cursor:pointer;">✕</button>
      </div>
...
function renderTravel() {
  ...
  trips.forEach(t => {
    ...
    if (t.members && t.members.length) {
      t.members.forEach(m => travelerCounts[m] = (travelerCounts[m] || 0) + 1);
  ...
  for (const person in travelerCounts)
    if (travelerCounts[person] > maxTrips) { maxTrips = travelerCounts[person];
      topTraveler = person; }
    ...
    el.innerHTML = trips.map(t => {
      ...
      return `
        <div class="row" style="padding:10px 16px;border-bottom:1px solid #e4e6ef;cursor:pointer;transition:background 0.2s;" onclick="focusTrip('${t.id}', ${t.lat}, ${t.lng})">
          ...
          <div class="row-main">
            <div class="row-title" style="font-weight:600;">${escapeHtml(t.city)}, ${escapeHtml(t.country)}</div>
            <div class="row-sub">${escapeHtml(t.date)} · Travelers: ${escapeHtml(membersStr)}</div>
            ${t.notes ? `<div style="font-size:11px;color:#888;margin-top:2px;font-style:italic;">"${escapeHtml(t.notes)}"</div>` : ''}
          </div>
      ...
      if (t.lat !== 0 || t.lng !== 0) {
        ...
        const popupContent = `
          <div style="font-family:sans-serif;padding:2px;min-width:150px;">
            <h4 style="margin:0 0 4px;color:#1a1a2e;font-size:13px;font-weight:bold;">📍 ${escapeHtml(t.city)}, ${escapeHtml(t.country)}</h4>
            <div style="font-size:11px;color:#9898aa;margin-bottom:6px;">📅 ${escapeHtml(t.date)}</div>
            <div style="font-size:12px;color:#5a5a72;margin-bottom:4px;"><strong>Who:</strong> ${escapeHtml(membersStr)}</div>
            ${t.notes ? `<div style="font-size:11px;color:#888;font-style:italic;border-top:1px solid #eee;padding-top:4px;margin-top:4px;">"${escapeHtml(t.notes)}"</div>` : ''}
          </div>`;
        ...
        const polyline = L.polyline(pathCoords, {
          color: pinColorClass === 'pin-blue' ? '#4f86c6' : pinColorClass === 'pin-amber' ? '#f4a261' : pinColorClass === 'pin-red' ?
            '#e05252' : pinColorClass === 'pin-green' ? '#3aaa75' : '#9b59b6',
          weight: 2,
          dashArray: '6,8',
...
function focusTrip(id, lat, lng) {
  if (!travelMap) return;
  travelMap.flyTo([lat, lng], 6, { animate: true, duration: 1.2 });
...
function playTravelTimeline() {
  ...
  function nextStep() {
    if (i >= sortedTrips.length) {
      ...
      setTimeout(() => { if (travelMap) travelMap.setView([20, 0], 2); }, 2000);
      return;
    }
...
async function fetchLocSuggestions(q) {
  try {
    const url =
      `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&accept-language=en&q=${encodeURIComponent(q)}&limit=5`;
    ...
    sug.innerHTML = data.map(item => {
      ...
      const safeCountry = JSON.stringify(country);
      const safeDisplay = JSON.stringify(display);
      return `<div class="autocomplete-suggestion" onclick="selectLocSuggestion(${safeCity}, ${safeCountry}, ${item.lat}, ${item.lon}, ${safeDisplay})">${escapeHtml(display)}</div>`;
    }).join('');
...
function renderUs() {
  ...
  if (marcusCheckin || eleanorCheckin) {
    ...
    if (marcusCheckin) marcusInfo = `
      <div style="font-size:18px; margin:4px 0;">${'❤️'.repeat(marcusCheckin.battery)}</div>
      <div style="font-size:10px; color:#a85f89;">Mood: ${escapeHtml(marcusCheckin.moods.join(', ') || 'Normal')}</div>
    ...
    if (eleanorCheckin) eleanorInfo = `
      <div style="font-size:18px; margin:4px 0;">${'❤️'.repeat(eleanorCheckin.battery)}</div>
      <div style="font-size:10px; color:#a85f89;">Mood: ${escapeHtml(eleanorCheckin.moods.join(', ') || 'Normal')}</div>
    ...
    `;
  } else alignmentHtml = '<div style="text-align:center; font-size:12px; color:#9898aa; padding:8px;">No check-ins logged yet. Make yours below!</div>';
  const historyCheckins = [...checkins].sort((a, b) => b.timestamp.localeCompare(a.timestamp));
  let historyHtml = '';
  if (historyCheckins.length === 0) historyHtml =
    '<div style="text-align:center; font-size:11px; color:#9898aa; padding:8px;">No past check-ins logged yet.</div>';
  else {
      ...
      return `
        <div style="padding:6px 0; border-bottom:1px solid rgba(253,222,228,0.3);">
          <div style="font-size:11px; display:flex; justify-content:space-between; align-items:center;">
            <span><strong>${escapeHtml(dateStr)}</strong> · ${userEmoji} ${escapeHtml(c.user)}</span>
            <span style="color:#ef4444; font-weight:700;">${'❤️'.repeat(c.battery)}</span>
          </div>
          ${c.moods && c.moods.length ? `<div style="font-size:10px; color:#a85f89; margin-left:14px; margin-top:2px;">Moods: ${escapeHtml(c.moods.join(', '))}</div>` : ''}
  ...
  container.innerHTML = `
    ...
    <div class="us-card">
      ...
      <div style="margin-top:10px;">
        <div style="font-size:12px; font-weight:700; color:#6b2d5c; margin-bottom:8px;">📬 Unlocked Notes from ${escapeHtml(partner)}</div>
...
function openAppreciationModal() { clr('love-note-msg');
  openM('m-love-note'); }
...
function setCheckinBattery(val) { selectedCheckinBattery = val;
  updateCheckinBatteryHearts(); }
...
function toggleCheckinHistory() {
  ...
  if (list) {
    if (list.style.display === 'none' || !list.style.display) { list.style.display = 'flex'; if (icon) icon.textContent =
        'Hide ▴'; } else { list.style.display = 'none'; if (icon) icon.textContent = 'Show ▾'; }
  }
}

// ─── Everything is now loaded ──────────────────────────────
console.log('✅ Wong Family app loaded successfully.');
